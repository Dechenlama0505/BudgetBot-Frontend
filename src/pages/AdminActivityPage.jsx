...
const AdminActivityPage = () => {
  ...
  useEffect(() => {
    const loadActivity = async () => {
      try {
        const response = await memberManagementAPI.listRecentActivity(100);
        const allActivity = Array.isArray(response?.data?.activity)
          ...
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-2 text-sm" style={{ color: textSub }}>
                Review all recent admin activity using the same dashboard card
                style.
              </p>
            </div>
          </div>
